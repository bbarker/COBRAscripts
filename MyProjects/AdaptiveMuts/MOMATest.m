function [solutionDel,solutionWT,totalFluxDiff,solStatus] = ...
  MOMATest(modelWT,modelDel,osenseStr,verbFlag,minNormFlag,objMult,WTflux,SPvec,RxnWeights)
...
% Solve wt problem
if minNormFlag
    %solutionWT = optimizeCbModel(modelWT,osenseStr,'one');
    solutionWT.x = WTflux;
    solutionWT.f = modelWT.c'*WTflux;
    solutionWT.stat = 1;
else
...
%Uncomment if using objMult ~= 1
        if nargin < 8
          SPvec = ones(1,length(modelWT.lb));
        else
          SPvec = ones(1,length(modelWT.lb)) + SPvec; 
        end
        %SPvec = SPvec(find(~modelDel.c));
        SPmax = max(SPvec);
	if nargin < 9
          RxnWeights = zeros(1,length(modelWT.lb));
	end
        b = zeros(nMets2,1);
        A = modelDel.S;
        c = -2*solutionWT.x;  
        c(find(modelDel.c)) = -objMult; 
        c(find(~modelDel.c)) = c(find(~modelDel.c)) .* SPvec(find(~modelDel.c))'/(objMult);
	%c = c .* RxnWeights';
        F = 2 * (diag(SPvec/(objMult)+log(log(objMult)+1)/(nRxns2/10) ));
	%To handle RxnWeights, we should wish to increase the propensity for a flux to
	%go in a particular direction, so it should suffice to alter the linear term additively
	c = c+RxnWeights';
        %!!!Assumes a single objective coeff:
        F(find(modelDel.c),find(modelDel.c)) = 0; %Previously was setting to objMult, 
        lb = modelDel.lb;                         %which worked since sol.f typically << 1,
        ub = modelDel.ub;                         %but now only positive-semidefinite.
        csense(1:nMets2) = 'E';
    ...
    if (QPsolution.stat > 0)
        if minNormFlag
            ...
            size(QPsolution.full)
            solutionDel.x = QPsolution.full((nRxns1+1):(nRxns1+nRxns2));
            solutionWT.x = QPsolution.full(1:nRxns1);
        end
        %disp(length(solutionDel.x))

        %Potentially dubious solution
        if length(solutionDel.x) < length(modelDel.c)
          solutionDel.x = 0 * modelDel.c;
        end
     
        solutionDel.f = sum(modelDel.c.*solutionDel.x);
	solutionDel.QPf = QPsolution.obj;
    ...
    %disp(strcat('FluxDiffMag: ',num2str((solutionDel.x' * solutionDel.x)/2 - ((solutionDel.f)^2)/2 + solutionWT.x' * solutionDel.x - solutionWT.f * solutionDel.f))); 
    %disp(strcat('BiomassMag: ',num2str(((solutionDel.f)^2)/2 + solutionWT.f * solutionDel.f)));   
else
