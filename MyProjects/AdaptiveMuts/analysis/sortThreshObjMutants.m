function mutMat = sortThreshObjMutants(model, grRateKOInc, VTensInc, WTensInc, grRateKODec, VTensDec, WTensDec, thresh)
...
if nargin < 8
  thresh = max(max(grTens));  
end

disp(size([VTensInc VTensDec]))
grTens = grTens .* (grTens<=thresh);
maxMVal = max(grTens');
