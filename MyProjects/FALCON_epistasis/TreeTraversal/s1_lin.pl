#!/usr/bin/perl -w
open(INP, $ARGV[0]); # original file for 4-gene file.
open(OUT, ">x1.txt"); # correlation coefficients between average initial fitness and slopes in each of 10,000  replicates for 4-gene-combination
...
$epistasis[$x]=(($p[$k][0][0]+$p[$h][0][0]+$p[$l][0][0])/3);
...
$bx=0;
$cx=0;
for ($hx=0; $hx<@epistasis; $hx++){
$ax+=(($epistasis[$hx]-$px)*($slope[$hx]-$py));
$bx+=(($epistasis[$hx]-$px)*($epistasis[$hx]-$px));
